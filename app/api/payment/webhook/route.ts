 ...
 * outer handler in {@link POST} translates that into a 200 ack.
 *
 ...
 *   {@link POST} catches and acknowledges.
 *
 ...
 * @throws Re-throws any non-`P2025` Prisma error so {@link POST} can either
 * acknowledge a `P2002` race or return 500 for genuine transient failures.
 */
 ...
 * @throws Re-throws any non-`P2025` Prisma error so {@link POST} can handle
 * concurrent-delivery races (`P2002`) or surface transient failures as 500.
 */
 ...
 */
async function handleEarlyFraudWarning(event: Stripe.RadarEarlyFraudWarningCreatedEvent) {
  const { object } = event.data
  const paymentIntentId = keyOf(object.payment_intent, 'id')
 ...
 *
 * Error-handling contract (see also {@link handleCheckoutComplete},
 * {@link handleDisputeCreated}, {@link handleChargeRefunded},
 * {@link handleEarlyFraudWarning}):
 *

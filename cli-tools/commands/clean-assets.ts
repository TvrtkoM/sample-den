...
const cleanAssets = new Command("clean-assets")
  ...
  .option(
    "--include-drafts",
    "Include draft samples when determining which assets are still in use"
  )
  .action(async ({ dryRun, force, includeDrafts }) => {
    log.header(
      includeDrafts
        ? "Fetching ALL sample documents (including drafts)…"
        : "Fetching ONLY published sample documents…"
    )
    ...
    const usedWavKeys = new Set(
      samples
        .map(s => s.highResFile?.s3Key)
        .filter((x): x is string => Boolean(x))
    )

    const usedMp3Ids = new Set(
      samples
        .map(s => s.previewFile?.asset?._ref)
        .filter((x): x is string => Boolean(x))
    )
